...
- **多角色环境隔离**：每个角色独立的 SOUL、模型配置、技能集与记忆；orchestrator 通过 Hermes Kanban 按名路由任务，default 作为外部 ingress 只做准入分流，不越权改代码。
...
见 [docs/hermes-profiles-guide.md 第 7 节](docs/hermes-profiles-guide.md) 的路线图：降低安装摩擦（跨平台检查、版本兼容矩阵）→ 证据机器校验（交接 Schema、SEB 完整性自动核验）→ 补齐编排平台能力（workspace 声明式路由、运行中取消、门禁降级）。
