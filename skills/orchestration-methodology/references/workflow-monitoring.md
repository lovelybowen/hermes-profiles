...
3. 同一质量门第二次失败时，重新分解任务。在再次尝试前检查简报是否模糊、角色分配是否错误、是否存在隐藏依赖或未知根因。**重新分解产生新计划，必须按 `references/decomposition-approval.md` 重过 DA 门**（plan_rev 递增，链式新审批卡）；重分解被无建议拒绝时按停止协议收尾（见该文件 §4.3）。
