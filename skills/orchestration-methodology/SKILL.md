...
持久化决策可使用 `scripts/decision_bridge.py`；它只记录单次决策并在批准后解锁绑定任务，不执行 push、merge 或 deploy。intake 快判的机械信号（文件数 / 行数 / 依赖 / 高风险路径）可用 `scripts/intake_signals.py` 计算——**辅助信号，不替代 orchestrator 判定，也不构成运行时自动分类器**。
