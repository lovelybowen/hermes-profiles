...
| T4 | 普通代码变更 | 小功能 / 修 bug，低爆炸半径 | 代码 | 高（git） | 低–中 | 是（L0 快速通道降为自验，§4.1） |
...
| R-moderate | 运行时配置或局部代码，爆炸半径低–中 | T3, T4 | **G1**（T3 触及生产/密钥/CI → **G2**；T4 加 QA 门，L0 快速通道降为自验 §4.1） | T4 是（L0 快速通道 → 自验） |
...
| T2 文档 | **G1 轻量异步** | L0（单文件 ≤30 行且无依赖）/ 否则 L1 | ≤ 8 轮 / ≤ 180s | 否 | 面向用户的政策文 |
| T3 配置 | **G1**；影响生产 / 密钥 / CI 则 **G2** | L1（接口 / 依赖 / schema 变化 → L1+L2） | ≤ 12 轮 / ≤ 300s | 视影响面 | 影响生产时（`Risk Approver`） |
| T4 普通代码 | **G1 轻量异步 + QA 门**（L0 快速通道见 §4.1） | L0（单文件 ≤30 行且无依赖）/ 否则 L1（可复用上游 L1+L2 / Full） | ≤ 12 轮 / ≤ 300s | 是；L0 快速通道 → 工程师自验 | 否（除非发现缺陷） |
...
- 验收标准**可判定**：能用一条命令或一个可观察行为判通过 / 不通过。

快速通道流程：

```
intake 快判（加载 references/intake-fast-path.md，产出六字段 intake 块）
  → 单张实现卡（engineer，Flow worktree）
  → 工程师自验：≥1 条命令 + exit code 写入交接 evidence（L0 最低证据强度）
  → orchestrator 以「工程师交接原文 + intake 块」作为 Flow 最终回复
```

纪律：

1. **降的是 QA 卡，不是证据要求。** 自验命令与 exit code 仍是 L0 的最低证据强度；缺命令的 L0 交接视为证据不足，退回补齐。
2. **门禁仍为 G1。** reviewer 依 G1 语义可选；跳过时按「跳过专家要记录原因」在交接中记录。
3. **实现中触发升级即退出快速通道。** 工程师发现改动将超出 L0 阈值（多文件、>30 行、需要动依赖 / 接口）时立即停止，在交接中标注 `needs_reclass: true` 交回 orchestrator 重新分类；不得「先做完再补材料」。
4. **事后抽样审计。** orchestrator 对已完成的 L0 任务抽样回溯（复算 blob hash / 复跑命令）；发现命中升级触发器的按 §8 升级处理，并计入 intake 校准指标（`rehearsal-comparison-metrics.md` §6）。
5. **分类不确定即不走快速通道**，按「信息不足保守升级」处理（§1）。
